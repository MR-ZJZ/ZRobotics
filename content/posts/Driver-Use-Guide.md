...
### 简介
电机驱动是机器人技术中软件硬件桥梁。项目针对永磁同步电机（PMSM）伺服控制，从实现**国产化替代**，**降低使用门槛**，**提高安全性**等角度设计完整的伺服软硬件系统。其使用了新型电流环结构，具备参数自辨识、编码线性补偿、无感控制、MIT力矩、轨迹规划、队列响应等适用于多场合的控制方案，具备离轴编码、双编码、多圈记忆等位置操作功能。
<br>

### 驱动硬件
#### 版本
| &emsp;&emsp;&emsp;Power&emsp;&emsp;&emsp; | &emsp;&emsp;&emsp;Joint&emsp;&emsp;&emsp; | &emsp;&emsp;&emsp;General&emsp;&emsp;&emsp; |
| :---------------------------------------: | :---------------------------------------: | :-----------------------------------------: |
|              **48V / 120A**               |               **48V / 60A**               |                **48V / 30A**                |

Power采用多叠层设计，功率层为铝基板，具备较大功率容量。Joint为关机电机设计，搭配可离轴编码器，实现双编码控制和刹车控制。General为基础版本，适用于大多数场合和学习。
<br>

#### 接口
| &emsp;USB&emsp; | &emsp;CAN&emsp; | &emsp;TTL&emsp; | &emsp;IO&emsp; | &emsp;SPI&emsp; |
| :-------------: | :-------------: | :-------------: | :------------: | :-------------: |
|     上位机      |    运行控制     |    简易通信     |     自定义     |     编码器      |

USB用于与上位机通信，进行调试和校准。CAN通信为主要通信方式，**具备最高的优先级**，用于实际运动控制。另外提供UART接口，SPI接口，用于简易通信和编码器。

### 软件参数
#### 用户参数
用户参数需要用户给定，包含电机特性，校准条件、控制增益、控制阈值等，可保存至Flash。
| 参数名称 | UART CMD ID | CAN CMD ID |     范围     |                                     含义                                     |
| :------: | :---------: | :--------: | :----------: | :--------------------------------------------------------------------------: |
| 启用无感 |     ses     |    0x01    |  [0,1] int   |                             0:禁用无感，1:启用无感                           |
|  编码器  |     enc     |    0x02    |  [0,7] int   |  0:禁用编码器 1:as5047p, 2:mt6816, 3:mt6825, 4:mt6835, 5:ma600, 6:icmu150, 7:admt4000    |
| 线性补偿 |     enl     |    0x03    |  [0,1] int   |                           0禁用线性补偿，反之启用                            |
| Node ID  |     cid     |    0x04    | [1,0xE] int  |                   CAN通信节点ID,最多14个节点，0xf为广播ID                    |
| CAN频率  |     cra     |    0x05    |  [0,2] int   |                          0:1MHz, 1:0.5MHz, 2:0.1MHz                          |
| 心跳间隔 |     cbe     |    0x06    | [0,1000] int |                       0表示无心跳，1表示1Khz，单位为ms                       |
| 电压下限 |     vbd     |    0x07    |   [10,55]    |                     电压低于该值将产生低电压报错，单位V                      |
| 位置上限 |     pou     |    0x08    |  [-1e4,1e4]  |                      位置大于该值将产生超位报错，单位圈                      |
| 位置下限 |     pod     |    0x09    |  [-1e4,1e4]  |                      位置小于该值将产生超位报错，单位圈                      |
| 速度限制 |     vli     |    0x0A    |  [0.01,1e4]  |         运行速度受该值制，速度绝对值大于该值将产生超速报错，单位rps          |
|  极对数  |     pol     |    0x0B    |  [1,50] int  |        电机极对数，用于编码器校准核验，错误填写将导致极对数不匹配报错        |
| 电流限制 |     ili     |    0x0C    |  [0.1,i_m]   | 运行电流受该值制，电流绝对值大于该值将产生过电流报错，i_m依据版本而定，单位A |
| 校准电流 |     icl     |    0x0D    |   [1,120]    |                  电机校准电流，尽量取额定电流的50％，单位A                   |
| 启动模式 |     ste     |    0x0E    |  [0,5] int   |                  启动后自动进入模式，非0时无需手动使能电机                   |
| 响应模式 |     acm     |    0x0F    |  [0,3] int   |    0:无反馈立即执行, 1:有反馈立即执行, 2:无反馈队列执行, 3:有反馈队列执行    |
| 滤波系数 |     fik     |    0x10    |  [0.001,1]   |          滤波系数，降低可以获得更平滑的速度，同时更容易产生控制滞后          |
| 容忍系数 |     tok     |    0x11    |  [1.1,3.0]   |          允许速度和电流短时间超过限制的裕度，大于该裕度立即产生报错          |
|  位置P   |     p_p     |    0x12    |    [0,50]    |                          位置比例增益，应小于速度环                          |
|  位置D   |     p_d     |    0x13    |   [0,100]    |                          位置积分增益，应小于速度环                          |
|  速度P   |     v_p     |    0x14    |    [0,50]    |                        速度比例增益，依据响应刚性给定                        |
|  速度I   |     v_i     |    0x15    |   [0,100]    |                          速度积分增益，消除静态误差                          |
|  加速度  |     acu     |    0x16    |   [0,1e4]    |           速度和位置运行加速度，0表示无穷大，单位圈/s<sup>2</sup>            |
|  减速度  |     acd     |    0x17    |   [0,1e4]    |           速度和位置运行减速度，0表示无穷大，单位圈/s<sup>2</sup>            |
...
| 参数名称 | UART CMD ID | CAN CMD ID |   范围    |                                                                                    含义                                                                                    |
| :------: | :---------: | :--------: | :-------: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| 相间电阻 |     rsi     |    0x19    | read-only |                                                           电机两相间电阻，用于构建电流环，合法范围为[10,15000]mR                                                           |
| 相间电感 |     ind     |    0x1A    | read-only |                                                           电机两相间电感，用于构建电流环，合法范围为[10,12000]uH                                                           |
|   KV值   |     k_v     |    0x1B    | read-only |                                                       电机每伏电压可以达到的转速，用于无感控制，合法范围为[10,5000]                                                        |
| 编码方向 |     dir     |    0x1C    | read-only |                                                            编码方向和电机正方向是否相同，1表示相同，-1表示相反                                                             |
| 编码偏移 |     eof     |    0x1D    | read-only |                                                              编码角度和电机电气角度的对齐偏差，范围为 [0,2pi]                                                              |
| 校准状态 |     cla     |    0x1E    | read-only | 电机当前的校准状态，0:未校准，1:仅校准编码器偏移，2:仅校准KV，3:已校准编码器偏移和补偿值，4:已校准编码器偏移和KV，5:完成所有校准，未完成校准进入对应控制模式将报错非法模式 |

#### 状态参数
状态参数反映电机运行参数，实现电机运动控制，方便用户调试，其不会被保存到Flash，包含用户设置的目标、实际运行值等。
|  参数名称  | UART  CMD ID | CAN CMD ID |    范围    |                                                                                            含义                                                                                            |
| :--------: | :----------: | :--------: | :--------: | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
...
|  电流输入  |     iin      |    0x22    | [-120,120] |                                                                                  电流模式下目标值，单位A                                                                                   |
|  速度输入  |     vin      |    0x23    | [-1e4,1e4] |                                                                                 速度模式下目标值，单位rps                                                                                  |
|  位置输入  |     pin      |    0x24    | [pou,pod]  |                                                                                  位置模式下的标值，单位圈                                                                                  |
|  队列控制  |     lif      |    0x25    | [0,16000]  |                                                             万位为启动标志，1表示启动，0表示停止执行。千位以下为执行队列点个数                                                             |
...
#### 部署过程要点
* 无感与有感控制<br>
无感适用于无位置传感器的场合，具备**速度波动小，噪声少**等优势，但其无法在零速范围内保持稳定。有感则具备全速范围适应性，但是其存在编码器噪声、非线性安装误差等因素带来的控制精度影响。可以根据需求合理选择，可以**同时开启有感和无感**，驱动器会在合适时刻切换位置参考，发挥两者优势。
* 编码非线性补偿<br> 
对于编码器**安装精度不高**或使用不具备**离轴**非线性校准的编码器时，推荐打开线性补偿。但是对于**齿槽转矩**较大的电机，编码补偿可能受到齿槽转矩谐波的影响，导致精度下降。应根据情况合理选择。
* CAN通信设置<br>
若总线存在其它协议设备时，注意配置合适的节点ID，**避免协议冲突**。最多支持14个节点，节点0xf为广播帧，节点0x0保留。可适当降低波特率来降低误码率。可设置CAN心跳反馈频率获取实时位置速度和电流。
* 电机参数设置<br> 
首先确定有无感控制模式、极对数和编码器,设置校准电流和最大电流，**推荐校准电流为额定的一半**。即可以进行参数校准。校准过程中可能产生各类错误，依据错误类型调整。如无错误，则系统不存在问题。可以进入使能控制。
* 控制参数调节<br> 
...
* 多圈位置记忆<br> 
可以通过上位机上的“保存位置”按钮实现对当前位置的锁存，在系统断电重启后将直接从该位置启动。若断电后电机产生移动，在±0.4圈内驱动器自动补偿，超过0.4圈时，将产生“初位变动”警告。

#### 运动控制要点
* 力矩控制<br>
力矩模式工作在MIT模式下，因此在扭矩模式给定设定电流的条件下不会持续旋转，若要持续加速，需要设置位置和速度增益为0。MIT模式有专用CAN帧格式。扭矩模式的最大速度受到最大转速(vli)参数的限制。
* 点动控制<br>
对于速度和位置的简单点动(从一个目标到另一个目标)运动控制，请设置合适加减速，驱动将自动完成规划。过大的加减速将对系统产生冲击。可以通过配置响应模式(acm)实现到点反馈can消息。
* 轨迹跟踪<br>
对于位置跟随要求较高的控制场合，有两种方式。其一是将加减速设置为0，同时上层控制器保持较高位置通信频率，实现简单跟随。对于频率受限场合，推荐使用PVT轨迹规划控制。在位置模式下，将队列间隔(pvt)设为控制帧间隔时间，即启动PVT模式，PVT模式有专用CAN帧格式。
* 队列控制<br>
...
| :----------------------------: | :----------------------------: | :------------------------------: |
| &emsp;清空队列并停止执行&emsp; | &emsp;清空队列并开始执行&emsp; | &emsp;保留1000点并开始执行&emsp; |
...
|   位置    |    速度    |    电流    | &emsp;位置增益&emsp; | &emsp;速度增益&emsp; |
| :-------: | :--------: | :--------: | :------------------: | :------------------: |
...
|   0x38   | NODE ID |  mod pin   |   mod vin   |   mod iin   |   mod p_p   |   mod v_p   |
```

实际电流环目标电流表示为

```yaml
p_p * (pin-pur) + v_p * (vin-vel) + iin
```

* **PVT轨迹跟踪帧格式**(**小端在左**)

```yaml
| ID b10~4 |  ID b3~0  | DATA Byte0~3 | DATA Byte4~7 |
|   0x39   |  NODE ID  |   mod vin    |    mod pin   |
```
...
|   0x3A   |  NODE ID  |   mod pur    |   mod vel    |   mod v_m    |
```
### 上位机
...
* 2区：用户参数区<br>
  所有用户需要给定的参数，用户根据需求调节，部分参数是点击**下拉窗口**，可通过后方三角按钮单独发送该数据，也可通过一键写入发送全部数据，**不发送则不会修改驱动器数据**，发送后如果需要永久保存，需要点击保存参数以保存到FLASH，鼠标放在2区内**滚动滚轮**可以拉出下方隐藏的部分非常用参数。
* 3区：模式和状态功能区<br>
  可以设置当前模式，显示和操作当前的模式、错误和警告。“测试模式”用于检测硬件是否异常，非必要不进入。“参数校准”用于自动获取电机参数。“释放电机”将对电机失能。“刹车急停”将迅速锁定当前位置，其将造成冲击，非必要不使用。对于校准数据，功能区也提供了查询接口，也可**写入当前位置和操作队列控制位**。
* 4区：绘图区<br>
  包含三个通道，可以在上拉窗口选择对应示波参数，单击三角按钮开始示波，再次单击停止。点击暂停示波后，鼠标放在该区域内滚动滚轮缩放图像，以图像中间为中心缩放，按住滚轮拖动鼠标调整中心位置，总数据为窗口数据的三倍。
* 5区：运动区<br>
  可以拖动滑条设置目标参数，鼠标在本区内拖动才有效。也可直接在右侧输入框输入后单击按钮发送。滑条的最大、最小值可以用户更改，为了避免误操作，**更改后回车才会有效**。

#### 基本操作流程

* 第一步：点击选择设备，选择正确设备后，点击后面连接图标，其变绿且出现状态信息时表示成功。选择了错误设备将出现“端口不匹配”的提示。

* 第二步：点击“一键读取”，读取设备信息，更新上位机内用户参数。同样可以一键写入将上位机内的用户参数写入驱动器。

* 第三步：根据控制对象设置用户参数的前8项内容，校准电流不宜过低，输出占空比低于3%将无法通过校准。点击3区的“参数校准”，电机开始运行自动完成校准程序。推荐在校准过程中打开电流、位置等监视信息。如校准过程产生错误，将在3区显示，若无错误，则表示校准完成。

* 第四步：点击“选择模式”后面的下拉窗口选择运行的模式，点击后三角按钮实现模式的进入，当前模式将被更新。如在速度模式，则可打开速度监视，在5区中拖动滑条或直接输入目标数据，电机将开始运行。调节适当的控制器增益和滤波条件，达到需求效果。

* 第五步：测试运行完成，点击“保存参数”完成对所有参数的锁存。驱动器自动重启，上位机断开。设备大概需要0.1s时间完成重启，重启完成后CAN会发送一帧自身节点ID消息。

#### Vofa兼容
上位机浮点协议兼容**Vofa**，其具备更强大的数据存储功能，如果需要长时间高速数据监测，可采用[vofa](https://www.vofa.plus/)，
驱动器可同时以约1khz的速度打印四个数据。控制方式为
```yaml
w_（通道）=（CAN CMD ID）\n
```
例如，通道a打印速度，
...
w_pwa=89\n
```
三个通道分别为pwa、pwb、pwc，pwd，89为0x59(vel)的十进制格式。关闭通道的方式为令通道值为0。
